...
# covers the state file, so one skipped stage is enough to prove the record is
# carried through unaltered; the evaluator has its own suite.
...
OUT=$(bash "$WRITER" "$STATE" implement 6 interactive 0 "2026-09-04T10:00:00Z" "$COND" 2>&1); ST=$?
assert_exit "fresh create succeeds (exit 0)" 0 $ST "$OUT"
assert_contains "reports the path written" "$STATE" "$OUT"
for pair in '"last_stage": "implement"' \
...
printf 'run: intake\nrun: deliver\n' > "$NOSKIP"
bash "$WRITER" "$TMPDIR_TEST/noskip.json" deliver 2 interactive 0 "2026-09-04T10:00:00Z" "$NOSKIP" >/dev/null
assert_contains "empty array is present" '"skipped": []' "$(cat "$TMPDIR_TEST/noskip.json")"

echo "[usage] the conditions file does not exist"
OUT=$(bash "$WRITER" "$TMPDIR_TEST/x.json" implement 6 interactive 0 "2026-09-04T10:00:00Z" "$TMPDIR_TEST/absent.txt" 2>&1); ST=$?
...
OUT=$(bash "$WRITER" "$NESTED" intake 6 interactive 0 "2026-09-04T10:00:00Z" "$COND" 2>&1); ST=$?
...
OUT=$(bash "$WRITER" "$STATE" deliver 6 interactive 1 "2026-09-04T10:00:00Z" "$COND" 2>&1); ST=$?
...
OUT=$(bash "$WRITER" "$TMPDIR_TEST/bad-total.json" implement six interactive 0 "2026-09-04T10:00:00Z" "$COND" 2>&1); ST=$?
...
OUT=$(bash "$WRITER" "$TMPDIR_TEST/bad-questions.json" implement 6 interactive zero "2026-09-04T10:00:00Z" "$COND" 2>&1); ST=$?
...
OUT=$(bash "$WRITER" "$TMPDIR_TEST/bad-mode.json" implement 6 curious 0 "2026-09-04T10:00:00Z" "$COND" 2>&1); ST=$?
...
OUT=$(bash "$WRITER" "$TMPDIR_TEST/bad-quote.json" 'implement"' 6 interactive 0 "2026-09-04T10:00:00Z" "$COND" 2>&1); ST=$?
