---
description: Detect this project's build/test/lint/serve commands and where it keeps specs and a local preview, propose marketplace plugins that plausibly match the detected stack as knowledge sources (never installed), confirm the detected values with you, and write them into .ai/project-config.yaml. Then detect this project's git remote, scan installed provider packs for a plausible match to the tracker, scm and design roles, confirm a choice with you, and write packs.tracker/packs.scm/packs.design. Then interview you for what detection cannot determine — where the units of work live, what makes a change done, stage conventions, the verification gate — and generate a platform pack plus its convention record from your answers. Re-runnable with keep / re-detect / edit at each stage. Never writes limits, never chooses packs.browser (no installed pack exists for that role yet), never installs anything, and never collects or writes a credential.
disable-model-invocation: true
---

You detect this project's own commands and layout, propose third-party plugins that might serve as knowledge sources for it, confirm the detected values with the human, and write them into `.ai/project-config.yaml` at the project root. Then you detect this project's git remote, scan the provider packs already installed alongside this core, propose which one plausibly serves the `tracker`, `scm` and `design` roles, confirm with the human, and write `packs.tracker` / `packs.scm` / `packs.design`. Then you interview the human for what detection cannot determine, and generate a platform pack — plus the written record of the interview's answers — from a template. The full shapes of these files are defined in `shared/project-config.md`, `shared/pack-manifest.md` and `shared/convention-record.md` — reference them, never restate them.

**What this skill does not do yet:** it never writes `packs.browser` or `limits` in `.ai/project-config.yaml` — the `browser` role is selected elsewhere, and `limits` is a human decision this skill never makes for you. A file written by this skill alone will not pass `shared/lib/validate-project-config.sh` until those land too; say so plainly in your final report rather than implying the file is complete. It also never writes `packs.platform` into that file — the generated pack's name lives in `.ai/project-conventions.yaml` for now, and gets wired into `packs.platform` by whichever future step first writes `packs` completely. It never asks for, collects, or writes a credential of any kind — a provider pack's own README states what it needs and where to get it.
...
## 5. Check for an existing provider-pack selection

Look at `.ai/project-config.yaml` for a `packs:` block already carrying non-empty `tracker`, `scm` and `design` values.

- **None is present:** go to step 6.
- **All three are present:** show them and ask, with `AskUserQuestion`: **keep** (stop here, nothing changes), **re-detect** (go to step 6 fresh, then confirm and overwrite through step 8), or **edit** (skip detection — ask the human directly which installed pack to use for each role, then go straight to step 7 with those).
- **Some are present:** treat it the same as re-detect for the roles that are missing, leaving the present ones alone unless the human chooses to change them too.

A file written by an earlier version of this skill carries `tracker` and `scm` but no `design` — that is the "some are present" case, not a malformed file.

## 6. Detect the git remote and scan installed provider packs

Two independent lookups; neither depends on the other succeeding.

- **Detect the remote.** Read the project's own git configuration for its remote host (for example, the host segment of `origin`'s URL). If none is configured or the lookup fails, say so plainly and continue — a detected host is useful context for step 7's proposal, not a requirement for this step to run.
...
- **Propose per role (`tracker`, `scm`, `design`).** The scan above is role-generic — it reads each pack's declared `role` — so a third role is a third pass over the same scan result, not a special case with its own detection. For each role, look at every installed provider pack declaring that role:
...
Before anything touches disk, list the `tracker`, `scm` and `design` values exactly as they will be written (or state plainly that a role stays unresolved this run), and ask the human to confirm or say what to change, using `AskUserQuestion`. Do not proceed to step 8 for a role until the human has explicitly accepted its value or explicitly agreed to leave it unresolved.

## 8. Write

`write-provider-packs.sh` writes `tracker`, `scm` and `design` together, as one block — it has no way to write one without the others, because it replaces the block wholesale rather than editing a line. Run it only once all three roles have a value: either confirmed in step 7 this run, or kept as-is from step 5 and carried forward unchanged.

```
${CLAUDE_PLUGIN_ROOT}/shared/lib/write-provider-packs.sh <config-path> <tracker> <scm> <design>
```

against `.ai/project-config.yaml` at the project root. If a role has no value at all this run — zero installed candidates and, for a first-ever run, nothing already in config — do not run the writer and do not invent a value for the other role either; report which role blocks the write and stop, per step 9. If the script exits non-zero, do not retry with a guessed substitute; report its output verbatim and stop.

## 9. Report (provider packs)

State plainly:

- the project-config path updated, and the `tracker`/`scm`/`design` values written into it — or, if the write did not run, which role blocked it and why (no installed candidate, or the human chose to leave it unresolved)
...
- that no credential was asked for, collected, or written at any point in steps 5-9 — point at the chosen pack's own README for what it needs and where to get it
- that a chosen pack may still need something on this machine that selecting it does not provide — a tool, or an authenticated session. This skill selects packs; it never installs anything and never verifies a pack's own preconditions.
- that `packs.platform`, `packs.browser`, and `limits` are still not part of `.ai/project-config.yaml`, so it will not pass the full config validator on its own

## 10. Check for an existing pack

Look for a convention record at `.ai/project-conventions.yaml`.

- **It does not exist:** go to step 11.
- **It exists:** show its five answers (pack name, unit-of-work location, definition of done, stage conventions, verification gate) and ask, with `AskUserQuestion`: **keep** (stop here, nothing changes — go straight to step 13 to report the existing pack), **re-generate** (go to step 11 fresh, then confirm and overwrite through step 12), or **edit** (skip straight to asking for a corrected value for each field the human names, then to step 12 with the full set).
...
- If `.ai/project-config.yaml` already carries a non-empty `packs.tracker` value, resolve that pack's `pack.yaml` the same way step 6 above resolves one — a `pack.yaml` at the root of an installed sibling plugin directory (a sibling of `${CLAUDE_PLUGIN_ROOT}`), declaring `kind: provider` and `role: tracker`. Read its `operations:` mapping for a `list_types` entry. If present, spawn the skill it names with `Skill(<skill name>)` and read its result envelope; take the item type names it reports as `<item-types>`.
...
- that `packs.platform`, `packs.design`, `packs.browser`, and `limits` are still not part of `.ai/project-config.yaml` at this point — `packs.tracker`/`packs.scm` were already written in steps 5-9 above; the rest still needs another step — so it will not pass the full config validator on its own
...
- **Detect first, ask second** — auto-detect everything a command value can be; ask only what genuinely cannot be determined (and always ask for `spec_dir`/`preview` if detection comes up empty, and for every step 11 answer detection cannot supply).
- **Never guess a path.** A blank `spec_dir` or `preview` is a contract violation, not a placeholder to fill in later. The same holds for every step 11 answer: an unanswered convention is a failed setup, never a pack with a hole in it (core contract §13 validator 10).
...
- **Never propose or write a provider pack that is not actually installed on disk.** A role with zero installed candidates gets a plain "none available" report (step 6) — never a guess, never a name carried over from a different role, never the pack's own description read as a hint at what to fabricate.
- **Never ask for, collect, or write a credential.** Steps 5-9 select a pack *name*; a chosen pack's own credentials are entirely its README's concern, named there and nowhere in this skill's own output.
