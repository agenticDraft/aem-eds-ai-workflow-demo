...
- **Neither is present:** go to step 6.
- **Both are present:** show them and ask, with `AskUserQuestion`: **keep** (stop here, nothing changes), **re-detect** (go to step 6 fresh, then confirm and overwrite through step 8), or **edit** (skip detection — ask the human directly which installed pack to use for each role, then go straight to step 7 with those).
...
Before anything touches disk, list the `tracker` and `scm` values exactly as they will be written (or state plainly that a role stays unresolved this run), and ask the human to confirm or say what to change, using `AskUserQuestion`. Do not proceed to step 8 for a role until the human has explicitly accepted its value or explicitly agreed to leave it unresolved.

## 8. Write

`write-provider-packs.sh` writes `tracker` and `scm` together, as one block — it has no way to write one without the other. Run it only once both roles have a value: either confirmed in step 7 this run, or kept as-is from step 5.
